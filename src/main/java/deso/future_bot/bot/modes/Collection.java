...
import deso.future_bot.bot.data.PriceWriter;
import deso.future_bot.bot.system.ConfigSetup;
import deso.future_bot.bot.trading.BuySell;
import deso.future_bot.bot.trading.CurrentAPI;
...
public final class Collection {
    ...
    public static final File backtestingFolder = new File("backtesting");
    ...
    private static boolean collectionInterface() {
        if (backtestingFolder.exists() && backtestingFolder.isDirectory()) {
            ...
            while (!input.equalsIgnoreCase("new")) {
                ...
                System.out.println("\nEnter \"new\" to start collecting a new deso.future_bot.bot.data file");
    ...
    public static void startCollection() {
        ...
        if (tempFolder.exists() && tempFolder.isDirectory()) {
            if (Objects.requireNonNull(tempFolder.list()).length > 0 && new File("temp/recovery").exists()) {
                ...
                System.out.println("Type \"compile\" to attempt to compile the downloaded deso.future_bot.bot.data or \"delete\" to delete them");
    ...
    public static boolean compileBackTestingData(long start, String filename) {
        System.out.println("\n---Writing deso.future_bot.bot.data from temp files to main file");
        try {
            Files.deleteIfExists(Path.of(filename));
        } catch (IOException e) {
        ...
        try (PriceWriter writer = new PriceWriter(filename)) {
            ...
            System.out.println();
            e.printStackTrace();
            System.out.println("\n---Could not compile backtesting deso.future_bot.bot.data into main file from temp files!");
            return false;
        }
        deleteTemp();
    ...
    public static void checkBacktestingData(String filename) {
        ...
        System.out.println("\n\n---Checking deso.future_bot.bot.data for consistency");
        ...
        try (PriceReader reader = new PriceReader(filename)) {
            ...
            while (bean != null) {
                if (bean.getTimestamp() < last) {
                    ...
                    if (firstReg) {
                        System.out.println("!--Date regression should never occour in deso.future_bot.bot.data. File an issue on https://github.com/markusaksli/TradeBot with your terminal history--!");
                        firstReg = false;
                    }
                }
    ...
    public static void describe(String filename) {
        try (PriceReader reader = new PriceReader(filename)) {
            ...
            System.out.println("-Longest gap in consistent deso.future_bot.bot.data: " + Formatter.formatDuration(max));
            ...
            while (true) {
                System.out.println("\nEnter \"back\" to return, \"check\" to verify the deso.future_bot.bot.data, and \"csv\" to create .csv file with price deso.future_bot.bot.data");
                String s = sc.nextLine();
                //TODO: Method to get csv with deso.future_bot.bot.indicators for ML (5min, interval, realtime)
